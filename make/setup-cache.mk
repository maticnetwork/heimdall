...
proto-dep-setup: $(CACHE)
	if [ ! -d "$(CACHE_COSMOS_SDK)" ]; then \
		CACHE_COSMOS_SDK_VERSION=$(shell grep 'github.com/cosmos/cosmos-sdk v' ./go.mod | sed 's/github.com\/cosmos\/cosmos-sdk//' | sed 's/^[[:blank:]]*//'); \
		git clone -b $$CACHE_COSMOS_SDK_VERSION --depth 1 https://github.com/cosmos/cosmos-sdk $(CACHE_COSMOS_SDK); \
	fi;
