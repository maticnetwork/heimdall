...
func GetSignBytes(ctx sdk.Context, chainID string, stdTx authTypes.StdTx, acc authTypes.Account, genesis bool) []byte {
	...
	fmt.Println("VAIBHAV ✅✅✅✅", "Pre Hardfork", "Current Height", ctx.BlockHeight(), "HardFork Block", helper.GetNewHexToStringAlgoHeight(), helper.GetNewSelectionAlgoHeight())
	...
	if bytes.Contains(signBytes, []byte(new)) {
		fmt.Println("VAIBHAV ✅✅✅✅", "Entered the check")
		signBytes = bytes.Replace(signBytes, []byte(new), []byte(old), 1)
	}

	fmt.Println("VAIBHAV ✅✅✅✅", "After", "SignBytes", string(signBytes))
