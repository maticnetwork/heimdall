...
func (cp *CheckpointProcessor) nextExpectedCheckpoint(checkpointContext *CheckpointContext, latestChildBlock uint64) (*ContractCheckpoint, error) {
	...
	_currentHeaderBlock, err := cp.contractConnector.CurrentHeaderBlock(rootChainInstance, checkpointParams.ChildBlockInterval, chainmanagerParams.ChainParams.RootChainAddress.EthAddress())
	if err != nil {
	...
	// get header info
	_, currentStart, currentEnd, lastCheckpointTime, _, err := cp.contractConnector.GetHeaderInfo(currentHeaderBlockNumber.Uint64(), rootChainInstance, checkpointParams.ChildBlockInterval, chainmanagerParams.ChainParams.RootChainAddress.EthAddress())
	if err != nil {
...
func (cp *CheckpointProcessor) getLatestCheckpointTime(checkpointContext *CheckpointContext) (int64, error) {
	...
	lastHeaderNumber, err := cp.contractConnector.CurrentHeaderBlock(rootChainInstance, checkpointParams.ChildBlockInterval, chainParams.RootChainAddress.EthAddress())
	if err != nil {
	...
	// header block
	_, _, _, createdAt, _, err := cp.contractConnector.GetHeaderInfo(lastHeaderNumber, rootChainInstance, checkpointParams.ChildBlockInterval, chainParams.RootChainAddress.EthAddress())
	if err != nil {
...
func (cp *CheckpointProcessor) shouldSendCheckpoint(checkpointContext *CheckpointContext, start uint64, end uint64) (bool, error) {
	...
	currentChildBlock, err := cp.contractConnector.GetLastChildBlock(rootChainInstance, chainmanagerParams.ChainParams.RootChainAddress.EthAddress())
	if err != nil {
