...
import (
	...
	"github.com/matiknetwork/heimdall/app"
...
func createRegenappAndExport(
	...
	var heimdallApp *app.HeimdallApp
	if height != -1 {
		heimdallApp = app.NewHeimdallApp(logger, db, traceStore, false, map[int64]bool{}, "", uint(1), encCfg)

		if err := heimdallApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		heimdallApp = app.NewHeimdallApp(logger, db, traceStore, true, map[int64]bool{}, "", uint(1), encCfg)
	}

	return heimdallApp.ExportAppStateAndValidators(forZeroHeight, jailAllowedAddrs)
}
