...
Example:
heimdalld testnet --v 4 --output-dir ./output --starting-ip-address 192.168.10.2
	`,
...
func InitTestnet(
	...
	heimdallConfig := srvconfig.DefaultConfig()
	heimdallConfig.MinGasPrices = minGasPrices
	heimdallConfig.API.Enable = true
	heimdallConfig.Telemetry.Enabled = true
	heimdallConfig.Telemetry.PrometheusRetentionTime = 60
	heimdallConfig.Telemetry.EnableHostnameLabel = false
	heimdallConfig.Telemetry.GlobalLabels = [][]string{{"chain_id", chainID}}
	...
	for i := 0; i < numValidators; i++ {
		...
		tokens, _ := sdk.NewIntFromString("1000000000000000000")
		coins := sdk.Coins{
			sdk.NewCoin(hmtypes.FeeToken, tokens),
		}
		...
		if err := writeFile(fmt.Sprintf("%v.json", nodeDirName), gentxsDir, nil); err != nil {
			return err
		}

		// write default heimdall config
		WriteDefaultHeimdallConfig(
			filepath.Join(nodeDir, "config/heimdall-config.toml"),
			helper.GetDefaultHeimdallConfig(),
		)

		srvconfig.WriteConfigFile(filepath.Join(nodeDir, "config/app.toml"), heimdallConfig)
	}
	...
	err := collectGenFiles(
		...
		outputDir, nodeDirPrefix, nodeDaemonHome, genBalIterator, peers,
	)
	if err != nil {
		return err
	}
...
func collectGenFiles(
	...
	for i := 0; i < numValidators; i++ {
		...
		// TODO: use gentxutil.GenAppStateFromConfig with gentxs and remove following code block
		// Heimdall change starts
		// populate persistent peers
		nodeConfig.P2P.PersistentPeers = persistentPeers
		nodeConfig.P2P.AddrBookStrict = false

		// overwrite default config
		cfg.WriteConfigFile(filepath.Join(nodeDir, "config", "config.toml"), nodeConfig)
		...
		nodeAppState, err := json.MarshalIndent(genDoc.AppState, "", "  ")
		if err != nil {
			return err
		}
		// Heimdall change ends
