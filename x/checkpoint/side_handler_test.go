...
//
//		// convert
//		//require.Equal(t, uint32(hmErrorTypes.CodeInvalidBlockInput), result.Code)
//		require.Equal(t, uint32(1501), result.Code)
//
...
//		//require.UnEqual(t, uint32(0), result.Code, "Side tx handler should fail")
...
//	suite.contractCaller.On("GetHeaderInfo", headerId, rootChainInstance, params.ChildBlockInterval).Return(header.RootHash, header.StartBlock, header.EndBlock, header.TimeStamp, header.Proposer, nil)
//
...
//		)
//
//		_,err  = suite.postHandler(ctx, &msgCheckpointAck, abci.SideTxResultType_NO)
//		require.NoError(t,err)
