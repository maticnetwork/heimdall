...
func SideHandleMsgValidatorJoin(ctx sdk.Context, msg types.MsgValidatorJoin, k keeper.Keeper, contractCaller helper.IContractCaller) (result abci.ResponseDeliverSideTx) {
	...
	// get main tx receipt
	// TODO remove hardcoded value 6 and use  params.MainchainTxConfirmations
	receipt, err := contractCaller.GetConfirmedTxReceipt(msg.TxHash.EthHash(), 6)
	if err != nil || receipt == nil {
		return abci.ResponseDeliverSideTx{Code: 2510} //hmCommon.ErrorSideTx(k.Codespace(), common.CodeWaitFrConfirmation)
	}
	...
	if !bytes.Equal(pubkey.Bytes()[1:], eventLog.SignerPubkey) {
		k.Logger(ctx).Error(
			...
			"mainchainValidator", hmCommonTypes.BytesToHexBytes(eventLog.SignerPubkey),
		)
...
// func PostHandleMsgValidatorJoin(ctx sdk.Context, k keeper.Keeper, msg types.MsgValidatorJoin, sideTxResult abci.SideTxResultType) sdk.Result {
...
// func PostHandleMsgStakeUpdate(ctx sdk.Context, k keeper.Keeper, msg types.MsgStakeUpdate, sideTxResult abci.SideTxResultType) sdk.Result {
...
// func PostHandleMsgSignerUpdate(ctx sdk.Context, k keeper.Keeper, msg types.MsgSignerUpdate, sideTxResult abci.SideTxResultType) sdk.Result {
...
// func PostHandleMsgValidatorExit(ctx sdk.Context, k keeper.Keeper, msg types.MsgValidatorExit, sideTxResult abci.SideTxResultType) sdk.Result {
