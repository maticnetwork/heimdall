...
func SplitKeyDeposit(key []byte) (proposalID uint64, depositorAddr common.HeimdallAddress) {
	return splitKeyWithAddress(key)
}

// SplitKeyVote split the votes key and returns the proposal id and voter address
func SplitKeyVote(key []byte) (proposalID uint64, voterAddr common.HeimdallAddress) {
	return splitKeyWithAddress(key)
}
...
func splitKeyWithAddress(key []byte) (proposalID uint64, addr common.HeimdallAddress) {
	...
	addr = commonTypes.BytesToHeimdallAddress(key[9:])
	return
}
